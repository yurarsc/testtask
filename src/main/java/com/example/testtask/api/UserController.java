...
public class UserController {
    ...
    @GetMapping
    public ResponseEntity<List<UserResponse>> search(@RequestParam(value = "dateOfBirth", required = false) LocalDate dateOfBirth,
                                                    @RequestParam(value = "phone", required = false) String phone,
                                                    @RequestParam(value = "name", required = false) String name,
                                                    @RequestParam(value = "email", required = false) String email,
                                                     @RequestHeader("X-Page-Number") Integer pageNumber,
                                                     @RequestHeader("X-Page-Size") Integer pageSize) {
        Page<User> page = userService.findUser(dateOfBirth, phone, name, email, pageNumber, pageSize);
